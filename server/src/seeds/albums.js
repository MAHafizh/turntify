  ...
  try {
    ...
    const getSongIds = (titles) => {
      const foundSongs = allSongs.filter(song => 
        titles.some(t => t.trim().toLowerCase() === song.title.trim().toLowerCase())
      );

      if (foundSongs.length !== titles.length) {
        const foundTitles = foundSongs.map(s => s.title.toLowerCase());
        const missingSongs = titles.filter(t => 
      ...
      return foundSongs.map(song => ({ song: song._id }));
    };
    ...
    console.log(JSON.stringify({
      ...
      artist: sampleAlbum.artist,
      releaseYear: sampleAlbum.releaseYear,
      totalSongs: sampleAlbum.songs.length,
      songs: sampleAlbum.songs.map(s => ({
        title: s.song.title,
        performer: s.song.performer
      }))
    }, null, 2));
